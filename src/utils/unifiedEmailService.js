...
export const sendUnifiedBookingEmail = async (bookingData) => {
  ...
  const isAirportService = bookingData.serviceType && 
    (bookingData.serviceType.includes('Airport') || 
     bookingData.serviceType.includes('Pickup') || 
     bookingData.serviceType.includes('Drop-off'));
  ...
  try {
    ...
    if (emailjsInstance && EMAILJS_CONFIG.SERVICE_ID !== 'YOUR_SERVICE_ID') {
      ...
      console.warn('EmailJS not configured properly, using mock service. To enable real emails, verify your EmailJS credentials in src/utils/unifiedEmailService.js');
